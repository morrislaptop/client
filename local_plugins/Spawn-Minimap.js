  ...
  async render(div) {
    ...
    const generate = () => {
      ...
      for (let i = radius * -1; i < radius; i += step) {
        ...
        for (let j = radius * -1; j < radius; j += step) {
          ...
          if (checkBounds(0, 0, i, j, radius)) {
            ...
            let perlin = df.spaceTypePerlin({ x: i, y: j });
            data.push({
              x: i,
              y: j,
              perlin,
              type: df.spaceTypeFromPerlin(perlin),
            });
          }
        }
      }
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i].perlin < df.contractConstants.INIT_PERLIN_MIN) {
          ctx.fillStyle = "#000000"; // too low, deep space
        } else if (data[i].perlin < df.contractConstants.INIT_PERLIN_MAX) {
          ctx.fillStyle = "#7d7d24"; // spawnable
        } else if (data[i].perlin < df.contractConstants.PERLIN_THRESHOLD_3) {
          ctx.fillStyle = "#000000"; // too high, deep space
        } else {
