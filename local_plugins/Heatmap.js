...
class Plugin {
  ...
  async render(div) {
    ...
    this.valuePlanetThreshold = div.querySelector("label.planetThreshold span");
    this.sliderPlanetThreshold = div.querySelector(
      "label.planetThreshold input"
    );
    this.sliderPlanetThreshold.addEventListener(
      "input",
      this.planetThresholdHandler
    );
  }
  ...
  planetThresholdHandler() {
    this.planetThreshold = parseFloat(this.sliderPlanetThreshold.value);
    this.valuePlanetThreshold.innerHTML = `${this.sliderPlanetThreshold.value}`;
    ui.viewportEntities.radiusPxMin = this.planetThreshold;
  }
  ...
  draw(ctx) {
    ...
    for (const p of planets) {
      ...
      gradient.addColorStop(
        1,
        `hsla(${hue},100%,${lightness}%, ${this.alpha})`
      );
  ...
  destroy() {
    ...
    this.sliderPlanetThreshold.removeEventListener(
      "input",
      this.planetThresholdHandler
    );
  }
